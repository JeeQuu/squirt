...
async function exportForSticker() {
    try {
        ...
        // Get start time from slider if video background
        let videoStartTime = 0;
        if (backgroundSprite?.texture.baseTexture.resource?.source instanceof HTMLVideoElement) {
            const video = backgroundSprite.texture.baseTexture.resource.source;
            const startFrameValue = parseInt(document.getElementById('start-frame').value);
            videoStartTime = (startFrameValue / 100) * video.duration;
            video.currentTime = videoStartTime;
    ...
    return new Promise((resolve, reject) => {
        ...
        video.onloadedmetadata = async () => {
            try {
                ...
                // Show start frame controls when video is loaded
                document.querySelector('.start-frame-controls').style.display = 'block';
                
                // Reset start frame slider and selected time
                const startFrameSlider = document.getElementById('start-frame');
                startFrameSlider.value = CONFIG.VIDEO.START_FRAME.DEFAULT;
                selectedStartTime = 0;
                lastVideoTime = 0;
                updateStartFrame(CONFIG.VIDEO.START_FRAME.DEFAULT);
                ...
                // Simplified ticker that only handles play state and texture updates
                pixiApp.ticker.add(() => {
                    if (video.paused) {
                        video.play().catch(console.error);
                    }
...
// Add this function to handle start frame changes
function updateStartFrame(value) {
    if (backgroundSprite?.texture.baseTexture.resource?.source instanceof HTMLVideoElement) {
        const video = backgroundSprite.texture.baseTexture.resource.source;
        selectedStartTime = (value / 100) * video.duration;
        video.currentTime = selectedStartTime;
        lastVideoTime = selectedStartTime; // Update lastVideoTime when manually seeking
        
        // Update display
        document.getElementById('start-frame-value').textContent = 
            `Start: ${selectedStartTime.toFixed(2)}s`;
    }
}